...
SplineRatesResults <- SplineRates_nest |> 
  select(nm, Well, mumax, rsquared, Source, SampleID, EthanolPercent)
...
MuEtOH_nest <- SplineRatesResults |>
  ...
  mutate(LinearFit = purrr::map(data, ~lm(mumax ~ EthanolPercent,
                                            data = .x)),
