...
ggplot(aes(x = Time_hour, y = OD, color = as.factor(SampleID), group = Well)) +
  geom_line() +
  geom_point() +
  facet_grid(rows = vars(EthanolPercent)) + 
#  labs(
...
SplineRates_nest <- Merged_ATData |> 
  group_by(nm, Well, Source, SampleID, EthanolPercent) |> 
  nest() |> 
...
SplineRatesResults <- SplineRates_nest |> 
  select(nm, Well, mumax_hour, rsquared, Source, SampleID, EthanolPercent)
...
MuEtOH_nest <- SplineRatesResults |>
  nest(.by = c("SampleID", "nm", "Source")) |> #want to nest by source
  mutate(LinearFit = purrr::map(data, ~lm(mumax_hour ~ EthanolPercent,
                                            data = .x)),
...
MuEtOH_nest |>
  ...
  ggplot() +
  geom_point(aes(x = EthanolPercent, y = mumax_hour)) +
  geom_line(aes(x = EthanolPercent, y = .fitted)) +
  #geom_point(aes(x = EthanolPercent, y = .resid), colour = "red") +
  facet_grid(cols = vars(Source , SampleID), rows = vars(nm)) +
  theme_bw()
```
